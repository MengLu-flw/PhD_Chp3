#check the directory
setwd("/Users/menglu/Library/CloudStorage/OneDrive-UniversityofEdinburgh/Edinburgh/2_Project/6_Methodology/2_Basic_stats/real/1_PCA/240117_Geum_symCF")
getwd()
...
# read in data
pca05 <- read_table("Geum_CFsym_05.eigenvec", col_names = FALSE)
eigenval05 <- scan("Geum_CFsym_05.eigenval")

pca10 <- read_table("Geum_CFsym_10.eigenvec", col_names = FALSE)
eigenval10 <- scan("Geum_CFsym_10.eigenval")
...
# keep cf samples only

pca05 <- pca05[pca05$class %in% c("G_rivale_cf", "G_urbanum_cf"),]

pca10 <- pca10[pca10$class %in% c("G_rivale_cf", "G_urbanum_cf"),]
...
pdffn = "Geum_cf_sym_SNP_PLINK_eigenvalues.pdf"
...
b <- b + scale_colour_manual(values = c("#B72FB8","#314A15"))
...
c <- c + scale_colour_manual(values = c("#B72FB8","#314A15"))
...
# print plots to a PDF
pdffn = "Geum_cf_sym_SNP_PLNIK.pdf"
